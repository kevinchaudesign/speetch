...
const OPTIONS: Option[] = [
  {
    ...
    tagline: "10 slides pitch deck pré-remplis",
    description:
      "Format YC / Sequoia : cover & vision, problème, solution, produit, marché, business model, traction, concurrence, équipe, ask. Une trame éprouvée pour raconter une histoire d'investissement.",
    status: "ready",
  },
  {
    id: "blank",
    ...
    description:
      "Un parchemin nu, à remplir comme tu veux. Tu peux ajouter des sections au fur et à mesure, ou copier-coller depuis Pitch.com / Figma.",
    status: "ready",
  },
  {
    id: "import-docx",
    ...
    description:
      "Confie un pitch existant au format Word. Mammoth convertit en HTML stylé, préservant titres, listes, gras/italiques et tableaux. Images embarquées inlinées.",
    status: "ready",
  },
  {
    id: "import-md",
...
function buildHref(
  ...
  if (option.kind === "structure") {
    return `${base}?template=pitch_deck&mode=create`;
  }
...
export function PitchDeckPicker({ projectName }: { projectName: string }) {
      ...
      <section className="mx-auto flex max-w-6xl flex-col gap-12 pt-20">
        <div className="flex flex-col gap-6">
          <p className="text-[11px] uppercase tracking-[0.4em] text-cyan-200/65">
            ...
            <span className="mx-3 text-cyan-200/20">·</span>
            <span className="text-cyan-200/85">Pitch deck</span>
          </p>
          ...
          <p className="max-w-xl text-balance font-serif text-base italic text-white/55 md:text-lg">
            Cinq voies pour forger un pitch deck : structure pré-remplie,
