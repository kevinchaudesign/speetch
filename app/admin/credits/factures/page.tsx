...
export default async function InvoicesListPage() {
  ...
  if (!user) redirect("/login?redirect=/admin/credits/factures");
  ...
  const { data } = await admin
    ...
    .select(
      "id, number, status, issued_at, due_at, client_name, client_company, total_ttc, paid_amount",
    )
      ...
      <section className="mx-auto flex max-w-6xl flex-col gap-12 pt-24 md:pt-20">
        <div className="flex flex-col gap-8">
          <p className="text-[11px] uppercase tracking-[0.4em] text-cyan-200/65">
            ...
            <span className="mx-3 text-cyan-200/20">→</span>
            <span className="text-cyan-200/85">Factures</span>
          </p>
          ...
          <div className="flex flex-wrap items-end justify-between gap-6">
            ...
            <Button href="/admin/credits/factures/new" variant="return">
              Émettre une facture
            </Button>
          </div>
        </div>
  ...
  return (
    <div className="relative flex flex-col items-start gap-8 pt-16">
      ...
      <p className="max-w-md text-balance font-serif text-base italic text-white/55 md:text-lg">
        Aucune facture émise. La première marquera le premier flux de
        crédits encaissés par l&apos;agence.
      </p>
      <Button href="/admin/credits/factures/new" variant="large">
...
function InvoicesTable({ rows }: { rows: InvoiceRow[] }) {
  return (
    <div className="overflow-hidden border border-cyan-200/15 bg-cyan-200/[0.015]">
      <div className="hidden grid-cols-[1.2fr_2fr_1fr_1fr_1fr_auto] items-center gap-4 border-b border-cyan-200/15 px-5 py-4 text-[10px] uppercase tracking-[0.32em] text-cyan-200/55 md:grid">
        <span>Numéro</span>
        <span>Client</span>
        <span>Statut</span>
      ...
      <ul className="divide-y divide-cyan-200/10">
        {rows.map((q) => {
          const status = isInvoiceStatus(q.status) ? q.status : "draft";
          return (
            <li key={q.id}>
              <Link
                href={`/admin/credits/factures/${q.id}`}
                className="group grid grid-cols-1 gap-2 px-5 py-5 transition-colors hover:bg-cyan-200/[0.04] md:grid-cols-[1.2fr_2fr_1fr_1fr_1fr_auto] md:items-center md:gap-4 md:py-4"
              >
                ...
                <span className="text-right font-mono text-sm text-emerald-300/85">
                  {formatEuro(Number(q.paid_amount))}
                </span>
