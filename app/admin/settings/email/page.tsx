...
export default async function EmailSettingsPage() {
  ...
  if (!user) redirect("/login?redirect=/admin/settings/email");
  ...
  const account = await loadOwnerEmailAccountMeta();
      ...
      <section className="mx-auto flex max-w-3xl flex-col items-start gap-12 pt-20">
        <div className="flex flex-col gap-6">
          <p className="text-[11px] uppercase tracking-[0.4em] text-cyan-200/55">
            ...
            <span className="mx-3 text-cyan-200/20">→</span>
            <span className="text-cyan-200/85">Boîte email</span>
          </p>
          ...
          <p className="max-w-2xl font-serif text-base italic text-white/55 md:text-lg">
            Connecte ta boîte <code className="not-italic text-cyan-100">contact@speetch.com</code>
            {" "}(Infomaniak) pour recevoir et envoyer des emails directement
            depuis le Conseil Jedi. Les identifiants sont chiffrés
            AES-256-GCM avant stockage.
          </p>

          {!keyConfigured && (
            <div
              className="rounded-md border border-amber-300/30 bg-amber-300/[0.05] px-5 py-4"
              style={{
                boxShadow: "0 0 16px rgba(252, 211, 77, 0.12)",
              }}
            >
              ...
              <p className="mt-2 font-serif text-sm italic text-white/65">
                Avant de configurer ton compte, ajoute{" "}
